...
const learningPackages={grundwissen:'Fahrschüler – Grundwissen',be:'B/BE – Vertiefung',a:'A – Motorrad',cde:'C/CE & D/DE – Nutzfahrzeug und Bus',fahrlehrer:'Fahrlehrer BE – Unterrichtsrelevant',alle:'Alle aktuellen Zeichen'};
const rootOf=s=>s.id.split('-')[0];
function belongsToPackage(s,key){if(!s.quiz||s.historical)return false;const root=rootOf(s);if(key==='grundwissen')return coreRoots.has(root);if(key==='be'||key==='fahrlehrer')return coreRoots.has(root)||extendedRoots.has(root);if(key==='a')return coreRoots.has(root)||extendedRoots.has(root)||/Kraftrad|Motorrad|Mofa/i.test(s.name);if(key==='cde')return coreRoots.has(root)||extendedRoots.has(root)||/Lastkraftwagen|Kraftomnibus|Kraftfahrzeug|Gewicht|Höhe|Breite|Länge/i.test(s.name);return true}
