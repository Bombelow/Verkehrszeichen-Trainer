...
const signs=[...new Map(raw.split('\n').map(line=>{const [id,name,category,extra]=line.split('|');const imageId=imageOverrides[id]??(reportedWithoutStaticSvg.has(id)?null:id);return[id,{id,name,category,extra:extra==='1',svg:imageId?`https://sevas.nrw.de/vz/${imageId}.svg`:null,quiz:verifiedQuizIds.has(id)&&!reportedWithoutStaticSvg.has(id)}]})).values()];
...
function detail(id){const s=signs.find(x=>x.id===id);shell(`VZ ${s.id}`,`<div class="detail"><div>${img(s)}</div><div><p><span class="badge ${s.extra?'extra':''}">${s.extra?'Zusatzzeichen':'Hauptzeichen'}</span></p><h2>${esc(s.name)}</h2><dl><dt>Nummer</dt><dd>VZ ${s.id}</dd><dt>Kategorie</dt><dd>${s.category}</dd><dt>Abbildung</dt><dd>${s.svg?`<a href="${s.svg}" target="_blank" rel="noreferrer">SEVAS-SVG öffnen</a>`:'Für dieses Zeichen ist keine statische SEVAS-SVG verfügbar.'}</dd></dl><button class="button ghost" id="back">Zur Bibliothek</button></div></div>`);document.querySelector('#back').onclick=()=>navigate('bibliothek')}
