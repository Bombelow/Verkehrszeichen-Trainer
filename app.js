...
function detail(id){const s=signs.find(x=>x.id===id);const official=s.svg?.startsWith('assets/')||s.svg?.includes('vzkat.de/');shell(`VZ ${s.id}`,`<div class="detail"><div>${img(s)}</div><div><p><span class="badge ${s.extra?'extra':''}">${s.extra?'Zusatzzeichen':'Hauptzeichen'}</span></p><h2>${esc(s.name)}</h2><dl><dt>Nummer</dt><dd>VZ ${s.id}</dd><dt>Kategorie</dt><dd>${s.category}</dd><dt>Abbildung</dt><dd>${s.svg?`<a href="${s.svg}" target="_blank" rel="noreferrer">${official?'Amtliche VzKat-Abbildung öffnen':'SEVAS-SVG öffnen'}</a>`:'Für dieses Zeichen ist keine statische SEVAS-SVG verfügbar.'}</dd></dl><button class="button ghost" id="back">Zur Bibliothek</button></div></div>`);document.querySelector('#back').onclick=()=>navigate('bibliothek')}
